# An Incomplete Reading List about Spectre and their Friends

## Speculative Execution Attacks
### 2018
[Reading privileged memory with a side-channel](https://googleprojectzero.blogspot.com/2018/01/reading-privileged-memory-with-side.html)

[Spectre-SSB: Speculative Store Bypass](https://bugs.chromium.org/p/project-zero/issues/detail?id=1528)

[Speculative Buffer Overflows: Attacks and Defenses](https://arxiv.org/abs/1807.03757)

[Spectre Returns! Speculation Attacks using the Return Stack Buffer](https://www.usenix.org/system/files/conference/woot18/woot18-paper-koruyeh.pdf)  
### 2019
[A Systematic Evaluation of Transient Execution Attacks and Defenses](https://www.usenix.org/system/files/sec19-canella.pdf)
### 2020
[Speculative Probing: Hacking Blind in the Spectre Era](https://download.vusec.net/papers/blindside_ccs20.pdf)

[Bypassing memory safety mechanisms through speculative control flow hijacks](https://arxiv.org/pdf/2003.05503.pdf)
### 2021
### 2022
[KASPER: Scanning for Generalized Transient Execution Gadgets in the Linux Kernel](https://download.vusec.net/papers/kasper_ndss22.pdf)

[RETBLEED: Arbitrary Speculative Code Execution with Return Instructions](https://comsec.ethz.ch/wp-content/files/retbleed_sec22.pdf)

[PHANTOM JMPS](https://comsec.ethz.ch/wp-content/files/retbleed_addendum_sec22.pdf)

[Branch History Injection: On the Effectiveness of Hardware Mitigations Against Cross-Privilege Spectre-v2 Attacks](https://download.vusec.net/papers/bhi-spectre-bhb_sec22.pdf)

## Relevant Exploit Techniques
[RAMBleed: Reading Bits in Memory Without Accessing Them](https://rambleed.com/docs/20190603-rambleed-web.pdf)

[Rage Against the Machine Clear: A Systematic Analysis of Machine Clears and Their Implications for Transient Execution Attacks](https://download.vusec.net/papers/fpvi-scsb_sec21.pdf)

[NetCAT: Practical Cache Attacks from the Network](https://download.vusec.net/papers/netcat_sp20.pdf)

[LVI: Hijacking Transient Execution through Microarchitectural Load Value Injection](https://ieeexplore.ieee.org/stamp/stamp.jsp?tp=&arnumber=9152763)
